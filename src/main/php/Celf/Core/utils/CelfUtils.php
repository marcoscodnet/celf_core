...
class CelfUtils {
	...
	const USERGROUP_RECLAMOS = 2;
	
	//AES
	const WLB_AES_PRIVATE_KEY = "w4l1b4";

	const USERNAME_EXTERNO = 'sistema.escritorio';
	...
	public static function aesEncrypt( $value ){
		...
		if(!empty($value))
			$encrypted = CoseUtils::aesEncrypt( $value, self::WLB_AES_PRIVATE_KEY );
		else
			$encrypted = "";
	...
	public static function aesDecrypt( $encrypted ){
		...
		if(!empty($encrypted))
			$decrypted = CoseUtils::aesDecrypt($encrypted, self::WLB_AES_PRIVATE_KEY );
		else	
			$decrypted = ""; 
		return $decrypted;
	...
	public static function isAsociado( User $user ){
	
		$usergroup = new Usergroup();
		$usergroup->setOid(self::USERGROUP_RECLAMOS);
		return $user->hasUsergroup( $usergroup );
	...
	public static function formatDateStr(\DateTime $value) {
		
		
		return $value->format("d").' de '.self::formatMesToView(intval($value->format("m"))).' de '.$value->format("Y");
	}
	...
	public static function sendMailPop($nameTo, $to, $subject, $msg, $attachs) {
		...
		/*$libsHome= CelfConfig::getInstance()->getLibsPath();
		...
		if( CelfConfig::TEST_MODE ){
			//me los envÃ­o todos a mi en test
			$to = CelfConfig::TEST_MAIL_TO;
		}
		...
		$mail->Host = CelfConfig::POP_MAIL_HOST;
		//Set the SMTP port number - likely to be 25, 465 or 587
		$mail->Port = CelfConfig::POP_MAIL_PORT;
		$mail->SMTPSecure = CelfConfig::POP_MAIL_SMTP_SECURE;
		//Whether to use SMTP authentication
		$mail->SMTPAuth = true;
		//Username to use for SMTP authentication
		$mail->Username = CelfConfig::POP_MAIL_USERNAME;
		//Password to use for SMTP authentication
		$mail->Password = CelfConfig::POP_MAIL_PASSWORD;

		//Set who the message is to be sent from
		$mail->setFrom(CelfConfig::MAIL_FROM, CelfConfig::MAIL_FROM_NAME);
		//Set an alternative reply-to address
		$mail->addReplyTo(CelfConfig::MAIL_FROM, CelfConfig::MAIL_FROM_NAME);
	...
	public static function sendMail($nameTo, $to, $subject, $msg, $attachs=null) {

		 
		 
		if (CelfConfig::MAIL_ENVIO_POP)
	...
	private static function getULStyle(){
	...
	private static function getLIStyle(){
	
		return "padding-left: 24px;padding-bottom: 2px;
    			background: url(https://www.waliba.com/miwaliba/css/images/logo-ul.png) 0em 0.5em no-repeat;
    			padding-top: 10px;margin-bottom: 1em;";
	}
	

	private static function getTituloStyle(){
		...
		return "width: 100%;
				...
				font-weight:bold;
				font-size:18px;
				background-color: #5AC6F3;
				color: #FFF;";
	}
	
	private static function getFirmaStyle(){
	...
	private static function getTextoStyle(){
	
		return "color:#FFF; background-color:#A8A9A9; padding:10px;border:1px solid #000;text-align:justify;";
	}
	
	private static function getResultadorStyle(){
	
		return "padding:5px;background-color: #F4F60C;";
		
	}
	
	public static function parsearEmailStyles( XTemplate $xtpl ){
	
		$xtpl->assign("ul_style", self::getULStyle() );
		$xtpl->assign("li_style", self::getLIStyle() );
		$xtpl->assign("titulo_style", self::getTituloStyle() );
		$xtpl->assign("texto_style", self::getTextoStyle() );
		$xtpl->assign("resaltador_style", self::getResultadorStyle() );
		$xtpl->assign("firma_style", self::getFirmaStyle() );
